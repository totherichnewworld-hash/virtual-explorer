...
| 档 | 画面 | 加载 | 要什么 |
|---|---|---|---|
| **照片** | Google 真实影像，贴地看会糊 | 最慢 | Google key |
| **白模** | Cesium OSM Buildings 全球白模 | 很快 | 免费 Cesium ion token |
| **轻量** | OSM 现查轮廓 + 卫星底图 | **最快** | **什么都不要** |
| **画风** | 真实轮廓 + 手绘立面，纸底版画感 | **最快** | **什么都不要** |

为什么差这么多：照片模式传的是**航拍重建的网格**（每块几百 KB 到几 MB），
后三档传的是**矢量轮廓**（一栋楼几十个坐标点）。同一个街区，前者几十 MB，后者几百 KB。

- **白模**需要 token，但代码已经写好了：面板里有个输入框，
  哪天你注册了 cesium.com（免费，不要信用卡）把 token 粘进去就直接能用，不用重新部署
- **轻量／画风**走 Overpass API 现查（三个镜像轮询），零注册。
  建筑高度取自 OSM 的 `height` 或 `building:levels` 标签，没标的按三层算
- 后三档的地面是**平的**（椭球面，零延迟零请求）—— 代价是里斯本的坡没了。
  想要真实地形就得切回照片档，或者再接一个高程数据源
...
- **手机体感**：拿着手机原地踏步 → 往前走；转身 → 转向（可选"跟真实指南针"，
  你面朝北，画面也面朝北）
- **点地面就走过去** —— 像 Street View，但不限于拍摄点：地面上任何一点都能点，
  相机会滑行过去并贴住那里的地面高度
- 手机：**单指拖动看四周、双指捏合缩放、点地面走过去、同一处连点两下回正**。
  页面本身的缩放被关掉了（含 Safari 的 gesture 事件）—— 否则捏合会放大整个网页，
  把 `position:fixed` 的面板挤出可视区，再也点不到
- 键盘：`W/S` 前后、`A/D` 转向、`R` 回正并重新校准、滚轮变焦、拖动看四周
- 一个输入框三种吃法：**搜地名**（走 OpenStreetMap 的 Nominatim，免费、不需要 key、
  不消耗 Google 配额）、粘 `纬度,经度`、或直接扔一条 Google Maps 链接。
  另有八个预设起点
...
瓦片是流式加载的：树要从根一层层往下钻，每层都得等上层元数据回来 —— 瓶颈是
**往返延迟 × 层数**，不是带宽。瞬移意味着缓存全空，整棵树重新钻；而贴地站着
又是最费瓦片的视角（近处要最高精度，地平线还拉得很远）。

这一版做了这些：

- **并发从 6 提到 18**。Cesium 默认每个服务器只开 6 条连接（HTTP/1.1 时代的保守值），
  而 Google 的瓦片服务器走 HTTP/2 多路复用 —— 这是单项最大的提速
- **`skipLevelOfDetail`**：不必把中间每一层都下完才显示，直奔目标精度，
  砍掉的正是那些串行往返
- **`progressiveResolutionHeightFraction: 0.5`**：先铺一版低清让画面尽快有东西
- **`foveatedConeSize: 0.15`**：屏幕中心优先加载，视野边缘可以慢
- **贴地不再抢带宽**：原本每 400 ms 调一次 `sampleHeightMostDetailed`，
  那个调用会强行拉取该点最高精度的瓦片，一直在和眼前的街景抢队列。
  现在默认用 `clampToHeight`（只读已渲染的瓦片，零请求），
  拿不到值才退回昂贵采样，且限流到 2.5 秒一次
- **手机上基准精度放到 24**（桌面 16）：小屏上肉眼几乎看不出，块数少一大截
- **画质开关**：面板里可以切「流畅／清晰」，直接把取舍交给你

另外：

- **空降**：跳转后先停在 260 m 高处、俯角 34°，2.4 秒内落到街面。粗瓦片几块就能
  铺满高空视野，下降过程中细节一层层补 —— 等待时间变成了降落本身
- **下降途中把 `maximumScreenSpaceError` 放到 32**，站定后收回 16：先要速度，后要细节
- **`dynamicScreenSpaceError`**：贴地视角下让远处瓦片自动降精度
- **缓存放大到 1 GB**：走回头路、来回切城市不用重下

另外修了一个跨城市跳转的 bug：空降动画原本每帧把地面高度写回出发城市的海拔，
把贴地采样的结果覆盖掉，导致落到京都时相机悬在一百多米高慢慢往下爬。
现在落差超过 25 m 直接对齐，只有走路中的小起伏才平滑跟随。
